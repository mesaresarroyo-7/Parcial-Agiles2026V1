    ...
    <aside className="sidebar">
      <div className="sidebar-header">
        <div className="sidebar-logo">
          <img 
            src="/logos/dollarcity-circular.png" 
            alt="DollarCity" 
            style={{width: '42px', height: '42px', borderRadius: '50%', flexShrink: 0}}
          />
          <div>
            <h1>DollarCity</h1>
            <span className="subtitle">{isProveedor ? 'Portal Proveedor' : 'Sede Santa Anita'}</span>
          </div>
        </div>
      </div>
      ...
      <nav className="sidebar-nav">
        {isProveedor ? (
          /* ===== MENÚ PROVEEDOR ===== */
          <>
            <span className="nav-section-title">Mi Panel</span>
            <NavLink to="/proveedor-dashboard" className={({isActive}) => `nav-link ${isActive ? 'active' : ''}`}>
              <span className="nav-icon">📊</span>
              Dashboard
            </NavLink>
          </>
        ) : (
          /* ===== MENÚ STAFF (admin, vendedor, almacenero) ===== */
          <>
            ...
            {usuario?.rol === 'admin' && (
              <>
                <NavLink to="/gestion-proveedores" className={({isActive}) => `nav-link ${isActive ? 'active' : ''}`}>
                  <span className="nav-icon">👥</span>
                  Gestión Proveedores
                </NavLink>
